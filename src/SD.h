...
class File_Base : public Stream {
private:
  char _name[13]; // our name
  SdFile *_file;  // underlying file pointer

public:
  File_Base(SdFile f, const char *name); // wraps an underlying SdFile
  File_Base(void);                       // 'empty' constructor
  ...
  File_Base openNextFile(uint8_t mode = O_RDONLY);
...
public:
  ...
  // write, etc). Returns a File_Base object for interacting with the file.
  // Note that currently only one file can be open at a time.
  File_Base open(const char *filename, uint8_t mode = FILE_READ);
  File_Base open(const String &filename, uint8_t mode = FILE_READ) {
